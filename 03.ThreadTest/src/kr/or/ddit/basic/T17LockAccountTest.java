...
/*
 	...
 	- ReentrantLock : Read 및  Write 구분없이 사용하기 위한 락 클래스로 동기화 처리를 위해 사용함
 					  synchroized를 이용한 동기화 처리보다 부가적인 기능이 제공됨
 					  ex) Fairness 설정 등 => 가장 오래 기다린 스레드가 가정 먼저 락을 획득함
 					  
	- ReentrantReadWhriteLock : Read 및 Write 락을 구분하여 사용가능 함
...
class LockAccount {
	...
	// Lock 객체 변수 선언 => 가급적 private final로 선언한다.
	...
	// 출금처리하는 메서드(출금성공 : ture, 출금실패 : false 반환)
	public boolean withdraw(int money) {
		boolean chk = false;

		// try ~ catch 블럭을 사용할 경우에는 unlock()메서드 호출은 finally 블럭에서 하도록 해야한다.
		try {
			lock.lock(); // 락 설정
